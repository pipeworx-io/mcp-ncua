...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/ncua_search_credit_unions \
  -H 'Content-Type: application/json' \
  -d '{"name":"navy federal"}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/ncua_search_credit_unions`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
